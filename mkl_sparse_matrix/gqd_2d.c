...
int main(){
    ...
    double hx,hy,Rx,Ry;
    double out;
    M = 4;             // x direction divide into M slice
    N = 4;             // y direction divide into N slice
    M2 = M/2;
    N2 = N/2;
        ...
        for(j=0;j<order;j+=1)
         ...
         //       out=(A[i*order+j].real)*(A[i*order+j].real)+(A[i*order+j].imag)*(A[i*order+j].imag)+
           //     (A[i*order+j+1].real)*(A[i*order+j+1].real)+(A[i*order+j+1].imag)*(A[i*order+j+1].imag);
            ...
            if (A[j*order+i].real==0&&A[j*order+i].imag==0) continue;
            else 
                printf("{%d: %f,%f}\n",j+1,A[j*order+i].real,A[j*order+i].imag);
            }
            //fprintf(v,"\n");
            printf("\n");
        }
